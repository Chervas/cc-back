...
- Ese mismo target incluye `campaign_quality`: extrae únicamente las campañas Google vinculadas a la estrategia y consulta mediante GAQL de solo lectura, en lotes de hasta 200 IDs por cuenta. Persiste estado/canal/puja/presupuesto/estado primario, métricas agregadas de los últimos 30 días, `optimization_score` cuando Google lo admite y la configuración de goal por campaña. Si una campaña no aparece en la consulta con métricas, hace una lectura de existencia sin métricas; si `optimization_score` no está soportado, repite sin ese campo. Una prueba de 201 campañas confirma dos lotes de calidad + dos de goals y 201/201 observadas. Detecta campañas ausentes, pausadas/eliminadas, limitaciones, score bajo, gasto sin conversiones y goals no alineados, y emite recomendaciones sin aplicarlas. La ausencia de mapping en una campaña pausada es warning.
...
- Evidencia live read-only final del 2026-07-16: 3 targets Conecta y mejora y 0 goal-policy. El grupo Propdental (`IntakeConfig #24`) quedó con medición saludable y `runtime_ready=true`: 8/8 destinos `validateOnly`, 0 críticos y un warning de renovación de attestation. Su calidad observó 30/30 campañas de ambas cuentas, con dos GAQL por cuenta, y leyó métricas, estado, puja, presupuesto, goals y `optimization_score`: 0 críticos y 85 incidencias/recomendaciones consultivas. Las campañas activas todavía heredan goals legacy de cuenta. Francia mostró 4/10 observables porque `599...` no está asignada a ese scope, y Eixample 0/4 porque ninguna cuenta está asignada a esa clínica. La auditoría no amplía permisos ni repara/muta Google.
