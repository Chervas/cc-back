...
El fix local preserva esas propiedades durante normalización, herencia y override. `node src/scripts/tests/google_ads_conversion_tracking.test.js` pasa y el readback del config efectivo devuelve `user_data_enabled=true` para ambos destinos. Esto prueba configuración, no un upload Enhanced nuevo: la aceptación real queda pendiente del primer lead natural consentido después del despliegue, con `user_identifier_count > 0` y diagnóstico terminal. No reescribir ni reenviar los leads anteriores para fabricar esa evidencia.
