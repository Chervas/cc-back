...
- **Mide y mejora** (`connect_only`): recopila, atribuye y audita las señales; envía a las cuentas configuradas las conversiones consentidas que correspondan y mejora la calidad de medición, pero no provisiona `CampaignOptimizationPolicy`, no ejecuta el lifecycle de umbrales ni cambia las pujas. Puede mostrar recomendaciones/readiness, pero no genera por sí solo una propuesta de transición. Para entrar en ese contrato debe existir una orden `managed_service` separada.
...
- El catálogo canónico está en `src/config/scheduledJobCatalog.js`: la misma regla durable se aplica a las 24 tareas periódicas. Estas y los cuatro backfills dirigidos comparten un único carril background global secuencial separado del CRM. Un backfill largo puede retrasar Diagnostics y revisiones; esta decisión prioriza que los syncs mutables no se solapen. El carril usa además `GET_LOCK` como defensa entre procesos, pero la topología soportada sigue siendo exactamente un worker `JobRequest` por `JOB_RUNTIME_NAMESPACE`: el lease no convierte `resetRunningJobs()` en multi-worker/HA. BullMQ continúa siendo la segunda capa para colas especializadas (WhatsApp, por ejemplo), pero ningún callback cron ejecuta negocio directamente.
