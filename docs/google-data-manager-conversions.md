...
El intento `#20` (`provider_request_id=6b1d7941-d6d2-4668-8b15-8e93be8748de`) seguía `accepted/provider_processing` en la lectura `JobRequest #23745` / `SyncLog #65219` de las `22:00:12 UTC`: `checked=1`, `processing=1`, `record_count=0`, sin errors/warnings. Envió un identificador `[email]`; el teléfono QA empezaba por `00` y quedó correctamente excluido al normalizarse a siete dígitos. Mientras Google responda `PROCESSING`, ese cero no es definitivo y no existe terminal ni atribución; `LeadIntake #7208` permanece intacto.
