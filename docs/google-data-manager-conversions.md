...
- Tras desplegar la corrección se reenvió el contacto únicamente a `Contact - ClinicaClick` de `1851215478`, con el mismo evento, click ID y consentimiento, sin PII. Google devolvió un nuevo `requestId`; tanto este intento correcto como el envío previo a Lead siguen `PROCESSING` en Diagnostics. El envío previo está identificado para su retirada mediante `ConversionAdjustment` en cuanto Google lo materialice; el `validateOnly` actual devuelve `CONVERSION_NOT_FOUND`, por lo que todavía no se ha ejecutado ninguna retirada a ciegas.
