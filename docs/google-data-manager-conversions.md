...
- Después de abrir el gate se repitió la ruta completa con una petición `validateOnly=true` por cuenta sobre las acciones Lead `7680195320` (`185...`) y `7540337982` (`599...`). Cada petición llevaba email y teléfono sintéticos, normalizados y hasheados, `adUserData=GRANTED` y `adPersonalization=GRANTED`; Data Manager devolvió aceptación y `requestId` en ambas. No se ingirió ninguna conversión ni se guardaron los hashes. Esto confirma el transporte Enhanced posterior al gate, pero no sustituye el diagnóstico de la próxima conversión publicitaria real consentida.
...
La misma auditoría no encontró custom goals de ClinicaClick, `CampaignOptimizationPolicies` activas ni `ChangeEvent` de hoy sobre las 19 campañas/recursos revisados. Todas las estrategias Propdental siguen `connect_only`; el único `ManagedCampaign` permanece `draft + observe`, sin aprobar. Por tanto, la medición no cambió campañas, goals, pujas, presupuesto o estado. El intento `#20` continúa `accepted/provider_processing` por su carril asíncrono y no altera esta prueba de no mutación.
...
- El catálogo canónico está en `src/config/scheduledJobCatalog.js`: la misma regla durable se aplica a las 30 tareas periódicas. Estas y cinco integraciones dirigidas —los cuatro backfills y `whatsapp_template_sync_delayed`— comparten un único carril background global secuencial separado del CRM. Las seis altas dev son retención PM2 y cinco bridges OPS UTC. Un backfill largo puede retrasar Diagnostics y revisiones; esta decisión prioriza que los syncs mutables no se solapen. El carril usa además `GET_LOCK` como defensa entre procesos, pero la topología soportada sigue siendo exactamente un worker `JobRequest` por `JOB_RUNTIME_NAMESPACE`. BullMQ continúa como segunda capa para transportes inmediatos; ningún callback cron ejecuta negocio directamente y los delays de horario silencioso/resync de plantillas permanecen en `JobRequests`.
...
El intento `#20` (`provider_request_id=6b1d7941-d6d2-4668-8b15-8e93be8748de`) seguía `accepted/provider_processing` en la lectura más reciente `JobRequest #23801` / `SyncLog #65272`, completada a las `00:34:26 UTC`: destino `PROCESSING`, `checked=1`, `processing=1`, `record_count=0`, sin errors/warnings. Envió un identificador `[email]`; el teléfono QA empezaba por `00` y quedó correctamente excluido al normalizarse a siete dígitos. Mientras Google responda `PROCESSING`, ese cero no es definitivo y no existe terminal ni atribución; `LeadIntake #7208` permanece intacto. La guía oficial admite hasta 24 horas de latencia, por lo que no se fuerza un terminal ni se limpia antes.
...
Objetivo pendiente: confirmar una conversión enriquecida atribuida en un evento natural posterior al fix, sin crear otro paciente, cita ni conversión sintética. Los recorridos naturales por GCLID ya quedaron cerrados para `lead` (`#7`) y `contact` (`#10`), pero eran pre-fix y no llevaban user data. Todavía no hay terminal natural de `qualified_lead` ni `schedule`: se observarán cuando el equipo verifique interés real o vincule una cita en CRM/Agenda, sin promover artificialmente un lead de prueba. No se debe reutilizar un click ID, PII o evento de otro visitante. El operador espera otro lead paid que llegue de forma natural desde una campaña activa, con consentimiento válido y click ID real, y lo sigue por su ID interno y sus hitos posteriores.
