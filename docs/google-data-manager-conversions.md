...
- El 2026-07-12 a las 23:31 UTC se crearon `Qualified Lead - ClinicaClick` en `5992356722` (`7682721076`) y `1851215478` (`7682299115`). Ambas son `UPLOAD_CLICKS`, categoría `QUALIFIED_LEAD`, `MANY_PER_CLICK`, habilitadas y secundarias (`primary_for_goal=false`, fuera de la métrica global). El `IntakeConfig` conserva destinos separados y cohortes de 4/15 campañas. Google Ads ya las devuelve por GAQL; Data Manager todavía responde `destination_references: Resource not found` durante la propagación. Las acciones Schedule de ambas cuentas siguen pasando `validateOnly`, por lo que el transporte y OAuth continúan correctos. Google documenta un mínimo de seis horas antes de reintentar una acción recién creada: <https://developers.google.com/google-ads/api/docs/conversions/troubleshooting>.
