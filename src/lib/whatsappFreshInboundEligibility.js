...
function eligible(message, conversation, binding, cutoff, now = Date.now()) {
  ...
  const reaction = message?.message_type === 'reaction' && m?.provider_type === 'reaction'
    && typeof m?.reaction?.emoji === 'string' && m.reaction.emoji.trim()
    && /^wamid\.[A-Za-z0-9+/=_:.-]{1,500}$/.test(m?.reaction?.message_id || '');
  return !!(binding?.sendEnabled === true && message?.direction === 'inbound' && (message.message_type === 'text'||media||reaction)
    ...
    && m.historical === false && !m.recovery_without_automation && !m.media_recovered_without_automation && (['text','button','interactive'].includes(m.provider_type)||media||reaction)
