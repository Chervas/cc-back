...
async function assertGoogleConversionMutationAccess({ userId, customerId, runtimeAccountId, transaction = null, models = require('../../models'),
  ...
  const mappings = await models.ClinicGoogleAdsAccount.findAll({ where: { isActive: true,
    ...
    attributes: ['id', 'clinicaId'], order: [['id', 'ASC']], raw: true,
  ...
  for (const mapping of mappings) {
    await assertAssetAccess({ userId, assetType: 'google.ads_account', assetId: mapping.id,
      ownerClinicId: mapping.clinicaId, assignmentModel: models.GroupAssetClinicAssignment, transaction,
      ...(transaction ? { authorizeClinicIds: options => require('./marketingScopeAccess').hasMarketingClinicScopeAccess({ ...options,
        membershipModel: { findAll: query => models.UsuarioClinica.findAll({ ...query, transaction, lock: transaction.LOCK.UPDATE }) } }) } : {}) });
  }
}
