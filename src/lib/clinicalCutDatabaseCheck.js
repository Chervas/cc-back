...
async function clinicalCutDatabaseCheck({ connect, plan, info, admissionClosed=false, stopped=false, expectedPending }) {
  ...
  try {
    ...
    const actual=await schema.snapshot(query),rows=await meta.rowFingerprints(query,plan.columns);
    meta.validate(plan,actual,rows,info,plan.database);
