...
function createIntegrationsBrokerClient({ origin, keyId, privateKey, audience, ca, timeoutMs = 15000, transportProfile = 'default' }) {
  ...
  return {
    execute(command, options = {}) {
      ...
      if (body.length > (ai ? aiLimits.MAX_REQUEST_BYTES : email ? emailLimits.MAX_REQUEST_BYTES
        : publicMedia ? publicMediaLimits.MAX_REQUEST_BYTES : 32768)) return Promise.reject(error('invalid_request'));
      ...
      return new Promise((resolve, reject) => {
        ...
        const req = https.request(new URL('/v1/execute', base), { method: 'POST', ca, rejectUnauthorized: true,
          ...
          const maxResponse=ai?aiLimits.MAX_RESPONSE_BYTES:email?emailLimits.MAX_RESPONSE_BYTES:publicMedia?publicMediaLimits.MAX_RESPONSE_BYTES:command.operation==='meta.whatsapp.authorized.media.read.v1'?Math.ceil(32*1024*1024/3)*4+4096:1048576;
