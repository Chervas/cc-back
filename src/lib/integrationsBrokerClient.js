...
function createIntegrationsBrokerClient({ origin, keyId, privateKey, audience, ca, timeoutMs = 15000, transportProfile = 'default' }) {
  ...
  try {
    ...
    if (base.protocol !== 'https:' || base.username || base.password || base.pathname !== '/' || base.search || base.hash
      ...
      || !['default', 'ai'].includes(transportProfile)) throw Error();
  ...
  return {
    execute(command, options = {}) {
      ...
      if (!Number.isInteger(budget) || budget < 1 || budget > maxTimeout
        || ai && !aiLimits.isAiOperation(command.operation)) return Promise.reject(error('invalid_request'));
      ...
      if (body.length > (ai ? aiLimits.MAX_REQUEST_BYTES : 32768)) return Promise.reject(error('invalid_request'));
      ...
      return new Promise((resolve, reject) => {
        ...
        const req = https.request(new URL('/v1/execute', base), { method: 'POST', ca, rejectUnauthorized: true,
          ...
          const maxResponse=ai?aiLimits.MAX_RESPONSE_BYTES:command.operation==='meta.whatsapp.authorized.media.read.v1'?Math.ceil(32*1024*1024/3)*4+4096:1048576;
