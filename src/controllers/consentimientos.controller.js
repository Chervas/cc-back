...
exports.syncClinicTemplates = asyncHandler(async (req, res) => {
    try {
        const result = await consentimientosService.syncClinicTemplatesFromCatalog(
            req.body?.clinic_id ?? req.body?.clinica_id ?? req.params.clinicId,
            getUserId(req)
        );
        return res.json(result);
    } catch (error) {
...
exports.listPatientDocuments = asyncHandler(async (req, res) => {
    try {
        const result = await consentimientosService.listPatientDocuments(req.params.id, req.query || {});
        return res.json(result);
    } catch (error) {
        return sendError(res, error);
    }
...
exports.exportPatientAudit = asyncHandler(async (req, res) => {
    try {
        const item = await consentimientosService.exportPatientConsentAudit(req.params.id, req.query || {});
        return res.json(item);
    } catch (error) {
        return sendError(res, error);
    }
