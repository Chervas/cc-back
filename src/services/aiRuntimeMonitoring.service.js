...
function normalizeUsageRow(row) {
  return {
    ...
    cached_input_tokens: Number(row.cachedInputTokens ?? row.cached_input_tokens ?? 0),
    audio_seconds: Number(row.audioSeconds ?? row.audio_seconds ?? 0),
    search_requests: Number(row.searchRequests ?? row.search_requests ?? 0),
    unpriced_requests: Number(row.unpricedRequests ?? row.unpriced_requests ?? 0),
...
function aggregateUsage(rows) {
  ...
  return {
    ...
    cached_input_tokens: sum(rows, 'cached_input_tokens'),
    audio_seconds: sum(rows, 'audio_seconds'),
    search_requests: sum(rows, 'search_requests'),
    unpriced_requests: sum(rows, 'unpriced_requests'),
...
async function getOverview({ force = false } = {}) {
  ...
  return {
    ...
    summary: {
      ...
      unpriced_requests_30d: sum(usage, 'unpriced_requests'),
    },
