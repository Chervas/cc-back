    ...
    async read({ actorId, sessionRef, query }) {
      ...
      return { version: 1, status: 'available', criteria, snapshot: state.snapshot, nextCursor,
        coverage: 'confirmed_platform_index_only', events: verified.map(v => ({ eventId: v.eventId, occurredAt: v.occurredAt,
          ...
          businessProfileMutation: v.version === 25 ? { operationRef: v.operationRef, mappingId: v.mappingId,
            assetRef: v.assetRef, family: v.family, executionRef: v.executionRef, nodeRef: v.nodeRef,
            initiatorUserId: v.initiatorUserId, clinicCount: v.clinicCount } : null,
          ...
          metaEnrollment: v.version === 24 ? { requestRef: v.requestRef, flowRef: v.flowRef, clinicCount: v.clinicCount, assetCount: v.assetCount, state: v.state } : null,
          ...
          googleReceiptReview: v.version === 19 ? { assetRef: v.assetRef, mappingId: v.mappingId, clinicCount: v.clinicCount,
            family: v.family, submissionRef: v.submissionRef, resultCount: v.resultCount, resultState: v.resultState } : null,
          googleDestinationList: v.version === 18 && v.action === 'integration.google_ads.destination_list'
            ? { assetRef: v.assetRef, mappingId: v.mappingId, clinicCount: v.clinicCount, resultCount: v.resultCount } : null,
          googleDestinations: v.version === 18 && v.action === 'integration.google_ads.destinations' ? { planRef: v.planRef, authorizationRef: v.authorizationRef,
            requestRef: v.requestRef, relatedCommandRef: v.relatedCommandRef, initiatorSessionRef: v.initiatorSessionRef,
            assetRef: v.assetRef, mappingId: v.mappingId, family: v.family, receiptState: v.receiptState,
            eventCount: v.eventCount, clinicCount: v.clinicCount } : null,
          adsActionPlan: v.version === 17 ? { planRef: v.planRef, requestRef: v.requestRef, relatedCommandRef: v.relatedCommandRef,
            initiatorSessionRef: v.initiatorSessionRef, assetRef: v.assetRef, mappingId: v.mappingId, family: v.family, mode: v.mode,
            receiptState: v.receiptState, closed: v.closed, eventCount: v.eventCount, clinicCount: v.clinicCount } : null,
          ...
          integrationDisconnect: [7, 9, 11].includes(v.version) ? { correlationId: v.correlationId, provider: v.provider,
            connectionRef: v.connectionRef, assetRef: v.assetRef } : null,
          integrationMapping: v.version === 12 ? { correlationId: v.correlationId, provider: v.provider,
            connectionRef: v.connectionRef, assetRef: v.assetRef, mappingId: v.mappingId, previousState: v.previousState, state: v.state,
            previousScope: v.previousScope, previousClinicId: v.previousClinicId, clinicId: v.clinicId,
            affectedClinicCount: v.affectedClinicCount, affectedClinicHash: v.affectedClinicHash } : null,
