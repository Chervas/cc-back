...
function exclusion(table) {
  return literal(['GoogleOAuthBrokerBindings', 'SearchConsoleBrokerBindings', 'AnalyticsBrokerBindings', 'GooglePropertyBrokerRevocations'].map(registry =>
...
function createGoogleLegacyCredentials({ connectionModel, bindingModel, searchConsoleModel, analyticsModel, propertyRevocationModel }) {
  ...
  async function check(connectionId, expectedSubject) {
    ...
    for (const registry of [bindingModel, searchConsoleModel, analyticsModel, propertyRevocationModel]) {
