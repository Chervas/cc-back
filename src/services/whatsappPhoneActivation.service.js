  ...
  async function persist(context,remote,actor){
    ...
    return locked(()=>db().sequelize.transaction(async transaction=>{
      ...
      if(existing){
        ...
        if(await db().ClinicMetaAsset.count({where:{assetType:'whatsapp_phone_number',phoneNumberId:remote.phoneId},transaction}))S.fail('whatsapp_authorization_conflict',409);
      ...
      if(existing?.state==='active')for(const key of ['routing','whatsapp_channel_role'])if(asset.additionalData?.[key])additionalData[key]=asset.additionalData[key];
      await asset.update({metaAssetName:remote.profile.displayPhoneNumber,waVerifiedName:remote.profile.verifiedName,
        quality_rating:remote.profile.qualityRating,isActive:state==='active',additionalData},{transaction});
      const values={authorization_id:context.requestId,asset_id:asset.id,scope_type:context.scope.type,scope_id:context.scope.id,
        ...
        state,profile:remote.profile,message_not_before:existing?.state==='active'?existing.message_not_before:
