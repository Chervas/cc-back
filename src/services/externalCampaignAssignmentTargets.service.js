...
function matchingIssueForAssignment(assignment, targetOptions) {
  ...
  if (!hasPointers) {
    ...
    code = 'target_invalid';
    message = 'El target guardado ya no pertenece a una estrategia activa Mide y entiende o Mejora de la misma clínica.';
...
async function resolveRequestedTarget({
  ...
  if (!isActiveConnectOnlyNewPatientsStrategy(request, campaign)
    ...
    throw assignmentTargetError(
      'matching_target_strategy_invalid',
      'El target debe pertenecer a una estrategia activa de Captar nuevos pacientes en Mide y entiende o Mejora.',
      409
    );
  }
