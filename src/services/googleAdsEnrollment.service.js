...
function createGoogleAdsEnrollment({ models, sessions, clients = configuredClients(), audit, now = Date.now,
  ...
  return {
    ...
    read: guarded(async (input, enrollmentId) => {
      if (gateway()) C.fail('google_ads_enrollment_disabled');
      const ids = actor(input); if (!C.UUID.test(enrollmentId)) C.fail();
      const m = getModels();
      return m.sequelize.transaction(async transaction => {
        ...
        const raw = await m.GoogleAdsEnrollmentRequest.findOne({ ...options, attributes: C.REQUEST_FIELDS,
          where: { enrollment_id: enrollmentId, scope_key: input.scopeKey } });
        if (!raw) C.fail('google_ads_enrollment_not_found'); const row = C.request(raw);
