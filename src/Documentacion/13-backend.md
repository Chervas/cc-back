...
- el aviso frontend v4 agrupa `ad_user_data` y `ad_personalization` bajo una única elección comprensible de **Marketing**. El backend no fusiona el contrato: conserva, valida y transmite ambas señales por separado, y la PII hasheada solo se habilita con `ad_user_data=GRANTED`;
...
- primer lead publicitario natural: `LeadIntake #7193`, Badalona `58`, `paid`, `web/tel_modal`. Su intento Contact `#10` hacia customer `1851215478` y action `7680195323`, con consentimiento `GRANTED` y GCLID, pasó `accepted -> SUCCESS`; Diagnostics devolvió un registro, cero warnings/errores y `attempt_count=1`, sin duplicado. Fue GCLID-only porque se creó antes del fix de ownership y no acredita identificadores Enhanced. La única evidencia pendiente es observar email/teléfono hasheados en el siguiente lead natural consentido, sin fabricar uno sintético.
