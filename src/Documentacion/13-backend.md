...
- Caché runtime V1: el servicio mantiene caché en memoria con TTL e in-flight dedupe para evitar segunda consulta pesada y colapsar llamadas concurrentes. Se cachean listado (`COMPETITION_REPORT_CACHE_TTL_MS`, default 3 min), sugerencias (10 min), provider status (1 min), Places Search (6 h), Place Details/fotos (12 h), heatmap (6 h) y Static Maps (6 h). Mutaciones (`create/update/delete/refresh`) limpian la caché local del proceso. La fuente autoritativa siguen siendo snapshots persistidos; la caché no sustituye al cron. El `competition_refresh` fuerza detalles de Places frescos para no reescribir snapshots con datos cacheados.
