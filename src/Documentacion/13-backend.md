...
La lectura pasiva de Competencia devuelve únicamente datos persistidos y nunca llama al proveedor ni encola el heatmap. Solo `Calcular mapa` / `Actualizar mapa` inicia la lectura dirigida. Si el backend responde `pending`, el frontend hace el GET inicial y como máximo cinco comprobaciones con backoff `4/7/12/20/30` segundos; después deja que el job durable continúe y evita polling permanente. `cache.refresh_available` no se deduce solo de que el estado sea distinto de `fresh`: con proveedor/gate contractual bloqueado debe ser `false`, sin `JobRequest`, polling ni llamada externa.
...
El inventario completo contiene 47 handlers. Las 5 integraciones dirigidas/background son `meta_ads_backfill_for_sites`, `web_backfill_for_sites`, `analytics_backfill_properties`, `business_profile_backfill_locations` y `whatsapp_template_sync_delayed`. También `automation_whatsapp_quiet_send` e `intake_quickchat_summary_materialize` usan `JobRequest`. Las esperas persisten `waiting + next_run_at`, deduplican y releen el activo al vencer sin guardar teléfono, texto ni token. BullMQ recibe únicamente transporte inmediato.
...
El fix desplegado de `effectiveMarketingAssets` preserva `user_data_enabled`, `enhanced_conversions`, `phone_country_code`, `value`, `consent` y `user_properties` en top-level, evento y destino; el readback devuelve user data activo en ambas cuentas. Los leads `#7200/#7202/#7203` fueron previos al fix y no acreditan identificadores. La prueba controlada post-fix `#22` sí registró `user_data_requested=true`, `user_data_sent=true`, dos hashes `[email, phone]`, un registro y cero warnings; terminó `INVALID_GCLID` porque el click ID era sintético. Esto acredita transporte Enhanced, no atribución. El intento `#20` sigue `PROCESSING` y la atribución real queda pendiente de un lead paid consentido con click ID válido.

### Capacidad de personal

`team.view` protege ruta, menú y lecturas de `/personal`; `team.manage` protege mutaciones de terceros. Los roles normalizados propietario/agencia/doctor/assistant/reception/admin_staff heredan lectura; `unknown` no. Por defecto `reception` y `admin_staff` heredan también `clinic.settings.edit` y `team.manage`, por lo que pueden gestionar configuración operativa de clínica, instalaciones, tratamientos, personal y horarios dentro de su ámbito. El backend de horarios y personal consulta `team.manage`, no solo el rol propietario/agencia, para que los defaults y overrides de `AccessPolicyOverrides` sean efectivos también al guardar. La gestión del horario propio conserva su excepción contractual.
...
- Personal separa lectura y gestión: `team.view` permite abrir/listar personal y horarios; `team.manage` protege mutaciones sobre terceros. La edición del horario propio conserva su comprobación específica y no se convierte en gestión de terceros. Los endpoints de invitaciones, horarios, bloqueos y excepciones de terceros aceptan propietario/admin global, default efectivo de `team.manage` para `reception`/`admin_staff`, u override efectivo de `team.manage` para la clínica.
...
- `GET /competition` devuelve `own_profile`, términos y datos manuales/snapshots autorizados. No consulta Meta/Google Ads en vivo. El ranking Places queda vacío mientras el gate contractual esté cerrado.
- `GET /competition/local-heatmap` devuelve `LOCAL_RANKING_PROVIDER_REQUIRED` por defecto. Si un proveedor compatible queda autorizado, calcula o recupera bajo demanda una matriz local para una búsqueda y radio (`1`, `3` o `5` km); el front **no** la lanza en la primera carga ni al cambiar configuración. No existe `map_image_data_url`: el frontend dibuja un grid abstracto y solo muestra `Google Maps` como atribución cuando el resultado procede realmente de Google.
- Caché preparada: `MarketingCompetitionHeatmapCaches` solo se lee/escribe con los gates de uso+almacenamiento activos. La clave cubre scope/ficha/término/radio/cuadrícula/algoritmo; dura 7 días `fresh` y hasta 14 `stale`. Un stale encola `marketing_competition_heatmap_refresh` con lease de 30 minutos/dedupe por clave y hasta 4 intentos. `system_data_cleanup` retira filas antiguas/versiones caducadas. La migración de purga deja cero contenido previo del proveedor.
...
- Mapa de calor local:
  ...
  - No se solicita, descarga, almacena ni reenvía Google Static Maps. La UI representa la medición como matriz abstracta; tampoco puede combinar contenido Places con OpenStreetMap u otra cartografía.
...
| Tabla | Uso |
|:---|:---|
| `MarketingCompetitors` | Competidores activos por clínica/grupo introducidos manualmente, con configuración opcional de página Meta. Las filas proveedor antiguas quedan purgadas/desactivadas. |
| `MarketingCompetitorSnapshots` | Snapshot histórico; el contenido Places anterior se elimina en la migración de cumplimiento. |
...
| `GET /api/marketing/reports/competition/suggestions` | Operativo con gate | Devuelve proveedor requerido + alta manual; solo sugiere automáticamente con licencia/gates explícitos. |
| `GET /api/marketing/reports/competition/local-heatmap` | Operativo con gate | Devuelve proveedor requerido; solo calcula bajo demanda con licencia/gates explícitos. |
...
- estado asíncrono aún abierto: `JobRequest #23801` / `SyncLog #65272` completó a las `00:34:26 UTC` con `checked=1`, `processing=1`; Google mantuvo el intento `#20` (`provider_request_id=6b1d7941-d6d2-4668-8b15-8e93be8748de`) en `accepted/provider_processing`, destino `PROCESSING`, `record_count=0`, sin errors/warnings. Solicitó y envió user data con un identificador `[email]`; el teléfono QA empezaba por `00` y el normalizador lo descartó correctamente al quedar en siete dígitos. No existe aún terminal ni atribución, ese cero durante `PROCESSING` no es definitivo y el scheduler será quien vuelva a comprobarlo. La guía oficial admite hasta 24 horas de latencia; no se fuerza un terminal ni se limpia antes;
- limpieza controlada: `LeadIntake #7207/#7209/#7210` y sus artefactos pasaron `dry-run -> simulate -> apply` con reconocimiento explícito de que borrar localmente no retracta eventos externos; el postcheck devolvió cero. Los naturales `#7184/#7193/#7194` permanecieron intactos. `#7208` y su auditoría se conservan mientras `#20` siga `PROCESSING`. La evidencia funcional que queda pendiente es doble: atribución Enhanced con un próximo lead paid real, consentido y con click ID válido, y terminales naturales de `qualified_lead`/`schedule` cuando el equipo avance un lead real a esos hitos. No deben simularse desde documentación ni inferirse de los cierres Lead/Contact.
...
- En solicitudes manuales de reseñas, `criteria.review_exclusion_rules.phone_prefixes` permite excluir países/prefijos internacionales antes de preparar el envío. El frontend envía prefijos normalizados sin `+` ni `00` (por ejemplo `33` para Francia) y `applyReviewRequestExclusions` marca esos items como `selected=false`, `status=excluded_phone_prefix`, `exclusion_reason=phone_prefix` y motivo `Prefijo telefónico +XX`. Estas exclusiones son solo de esa solicitud/lista y no crean bajas globales.
