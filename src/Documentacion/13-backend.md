...
- Mapa de calor local:
  ...
  - Cada tile solo necesita conocer si el `place_id` propio aparece y en qué índice. Su Text Search usa `X-Goog-FieldMask: places.id`, `pageSize=20`, una llamada por punto y sin fallback. Una única Text Search Pro cacheada por identidad de mapa pide `places.id,places.displayName` para etiquetar los demás puestos; no se repite en las 25 celdas. Esto mantiene la matriz `5x5` y Top 20 sin multiplicar el SKU nominal. Con la configuración por defecto una caché nueva hace 25 Text Search IDs Only, como máximo 1 búsqueda nominal y, solo si faltan coordenadas persistidas, 1 Place Details `id,location` adicional.
