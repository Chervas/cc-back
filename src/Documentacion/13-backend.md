...
- estado asíncrono aún abierto: `JobRequest #23745` / `SyncLog #65219` completó a las `22:00:12 UTC` con `checked=1`, `processing=1`; Google mantuvo el intento `#20` (`provider_request_id=6b1d7941-d6d2-4668-8b15-8e93be8748de`) en `PROCESSING`, `record_count=0`, sin errors/warnings. Solicitó y envió user data con un identificador `[email]`; el teléfono QA empezaba por `00` y el normalizador lo descartó correctamente al quedar en siete dígitos. No existe aún terminal ni atribución, ese cero durante `PROCESSING` no es definitivo y el scheduler será quien vuelva a comprobarlo;
