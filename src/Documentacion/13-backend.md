...
- `GET /api/pacientes/:id/nutrition-measurements/:measurementId/report/render` y `/report/pdf` aceptan `compare_measurement_id=<measurement_id>` para renderizar HTML/PDF contra una medicion concreta y `compare_measurement_id=none` para generar el documento sin comparativa. Cuando se usa este parametro, el backend renderiza bajo demanda y no reutiliza ni reescribe snapshots finales; asi la UI puede cambiar la comparacion sin romper la inmutabilidad clinica del informe cerrado.
- El HTML/PDF de Nutricion debe ser entendible por paciente no tecnico: fraccionamiento molecular/tisular incluye explicaciones cortas, la distribucion adiposa/muscular diferencia `Actual` y `Comparacion` y la somatocarta mantiene etiquetas y valores separados para evitar solapes. La imagen central de distribucion solo localiza zonas; las barras son la fuente visual de valores.
