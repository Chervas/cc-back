...
- Personal separa lectura y gestión: `team.view` permite abrir/listar personal y horarios; `team.manage` protege mutaciones sobre terceros. La edición del horario propio conserva su comprobación específica y no se convierte en gestión de terceros.
...
- prueba negativa de mutación en el mismo corte: cero custom goals ClinicaClick, cero `CampaignOptimizationPolicies`, cero `ChangeEvent` de hoy sobre las 19 campañas/recursos revisados, todas las estrategias Propdental en `connect_only` y el único `ManagedCampaign` en `draft + observe`, no aprobado. `#20` seguía `PROCESSING` por separado. No se cambiaron campañas, goals, pujas, presupuesto o estados.
...
- Regresión canónica: `node src/scripts/tests/intake_config_write_merge.test.js`. Debe conservar `google_ads.goal_policy`, unknown fields, Enhanced, valores/destinos y comprobar locks, mutación estrecha y proyección pública saneada.
