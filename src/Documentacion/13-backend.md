...
- `Conversations.unread_count` se mantiene por compatibilidad. La UI separa dos estados calculados en lote y sin N+1: `unread_count` cuenta inbound visibles posteriores a la lectura de ese usuario y al último outbound válido; `pending_automation_count` conserva los inbound posteriores al último outbound solo cuando hay una `Notifications.event=automation.system_notification` abierta para `data.quickChatConversationId`. Abrir hace `ConversationReads.upsert`, limpia únicamente el azul del usuario y emite `conversation:read` a `user:{id}` sin agregado global. Un outbound desde CRM o un eco móvil limpia el pendiente para todos. La migración `20260721143000-index-notification-automation-attention.js` indexa `event + is_read`.
