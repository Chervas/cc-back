...
- `GET /api/marketing/review-requests/summary` acepta `preview_limit`/`previewLimit` para ampliar la muestra de `candidates_preview` hasta 250 filas sin cambiar la preparación/envío real. La respuesta añade `approved_photo_template_available` y `approved_photo_template_id`: si hay `review_team_photo_url` pero este valor es falso, el envío seguirá usando la plantilla de texto aprobada hasta que exista una variante WABA `clinicaclick_solicitar_resena_foto*` aprobada con cabecera `HEADER/IMAGE`.
