...
- `GET /api/marketing/review-requests/summary` incluye `low_rating_reasons` como panel operativo de valoraciones privadas recientes `1-5`, no como reseña pública. Cada fila devuelve `patient_id`, `conversation_id`, `clinic_id` y `clinic_name` para filtrar por sede y abrir el paciente en QuickChat/info desde el frontend. Para valoraciones `5/5` sin comentario se muestra un texto operativo neutro; para `1-4` se cruza el motivo privado si el paciente lo respondió. Las métricas y motivos recientes se atribuyen solo a listas/campañas con `criteria.review_request = true` o `template_usage = solicitud_resena`, y excluyen eventos `mass_campaign_test_*` para no medir conversiones de pruebas.
