...
- la disponibilidad, la previsualizacion y el envio de reseñas deben resolver la misma WABA efectiva mediante el proposito `review_requests`. No se puede declarar lista una clinica usando una plantilla aprobada del primario y enviar despues por el secundario. Una version personalizada creada por un administrador puede ser la plantilla efectiva si conserva el contrato de reseña, esta `APPROVED` en esa WABA y usa la categoria finalmente aceptada por Meta, incluida `UTILITY`;
...
- Si Meta devuelve `GraphMethodException code=100 error_subcode=33` al enviar o leer un numero en coexistencia, el backend marca `ClinicMetaAssets.additionalData.coexistence.status=disconnected`, `canSendApi=false`, `requiresReconnect=true` y emite la notificacion `whatsapp.coexistence_disconnected` con accion `Reconectar WhatsApp` hacia `/ajustes?tab=whatsapp`. La proyeccion se mantiene igual tanto en el activo canonico `whatsapp_phone_number` como en su fila WABA hermana para que ninguna vista muestre un verde antiguo; el panel general toma el estado del numero como autoridad cuando existe. Esto cubre sesiones compartidas caducadas/inactivas, la app de ClinicaClick desinstalada del WABA o tokens que pierden permisos sobre el WABA/phone. El texto operativo indica reconectar desde Ajustes y, en modo compartido, abrir WhatsApp Business en el movil, escribir un mensaje desde ese movil y recibir respuesta antes de cerrar la alerta;
...
- Google Ads Transparency:
  ...
  - La lectura es resiliente ante intentos fallidos ya persistidos: si el snapshot más reciente está `unavailable|error|failed`, el informe usa como datos el último snapshot `completed` y expone por separado `data_is_stale`, `refresh_status`, `refresh_error_*` y `refresh_attempted_at`. Así un `429` posterior nunca tapa un recuento o unas creatividades válidas con un falso cero, pero la UI puede avisar de que la renovación sigue pendiente.
...
- Mapa de calor local:
  ...
  - `GET /competition/local-heatmap/searches` devuelve en cada búsqueda una serie `evolution` con hasta 12 mediciones recientes de posición media. El backend resuelve todas las tarjetas con una única lectura acotada de snapshots —máximo 500 filas y 180 días por defecto— y nunca hace una consulta N+1 por término/radio.
...
- El schedule semanal solicita `googleTransparencyMode=official_global`. El
  ejecutor acepta el lote oficial únicamente con scope `all`, deduplica por
  semana y no repite un ciclo ya completado. El flag semanal está apagado por
  defecto para impedir que dev, staging o un segundo worker dupliquen el scan.
...
- `COMPETITION_GOOGLE_ADS_TRANSPARENCY_ENABLED=false` desactiva Google Ads Transparency sin tocar el resto del informe. El fast-path no usa credencial y está acotado por `COMPETITION_GOOGLE_ADS_TRANSPARENCY_FAST_PATH_ENABLED`, `..._FAST_AD_LIMIT` y `..._TIMEOUT_MS`. Un `429` abre un circuito común al proceso (`..._FAST_COOLDOWN_MS`, 15 minutos por defecto): durante ese intervalo no se vuelve a llamar al endpoint público, se conserva el snapshot anterior y el lote semanal queda como vía de recuperación. Una URL o ID manual de anunciante se persiste en `raw_place_payload.clinicaclick_google_ads` y tiene prioridad exacta tanto en el fast-path como en BigQuery. El lote oficial requiere `GOOGLE_APPLICATION_CREDENTIALS` —o JSON/base64 equivalente fuera de Git— y `COMPETITION_GOOGLE_ADS_TRANSPARENCY_WEEKLY_ENABLED=true` solo en el worker líder. `COMPETITION_GOOGLE_ADS_TRANSPARENCY_MAX_BYTES_BILLED` fija el techo duro; su valor seguro por defecto es `25000000000`.
...
- `MarketingPatientLists` y `MarketingPatientListItems`: congelan la audiencia importada, estado de cada fila, motivos de exclusión y variables disponibles para la campaña. En `mass_sends`, `criteria.record_kind` distingue `recipient_list` de `campaign`; la campaña conserva `criteria.source_list_id` y copia los items para no mutar la lista reutilizable.
...
| POST | `/campaigns` | Crea una lista (`record_kind=recipient_list`) desde importacion/manual/pacientes actuales o una campaña (`record_kind=campaign`) copiando `source_list_id`. Acepta `campaign_name`, `template_usage`, `template_commercial`, `opt_out_text` y `link_tracking` en `criteria`. |
...
- Una campaña creada desde `source_list_id` recibe una copia nueva de los destinatarios y recalcula la elegibilidad para sus canales. La copia elimina IDs de mensajes, conversación, estado de dispatch y fechas de entrega; la lista de origen nunca se convierte ni se actualiza como campaña.
- Archivar una campaña no archiva su lista de origen. Si una campaña legacy no tiene `source_list_id`, `PATCH ... status=archived` materializa primero una `recipient_list`, enlaza `source_campaign_id`/`preserved_recipient_list_id` y elimina de la copia cualquier estado o identificador de envío.
- `prepare`, `test-send`, `start` y `resume` rechazan con `409` los registros `record_kind=recipient_list`. Los registros históricos sin `record_kind` se consideran campañas para compatibilidad.
...
- Campañas Admin expone `GET/PUT /api/admin/campaign-playbooks/bulk-send-settings` para configurar ajustes de envíos masivos WhatsApp: batch size, delay y baja máxima. `prepare` guarda snapshot en `criteria.dispatch`; el delay mínimo efectivo es 2 minutos por lote. Email usa `email_dispatch`, su propio tamaño/espera de lote y `JobRequest.email_send`, sin compartir el cursor de WhatsApp.
- En campañas de email, `criteria.email_subject` y `criteria.email_preheader` pertenecen a la campaña y prevalecen sobre los valores históricos de `MarketingEmailTemplate`; el cuerpo sigue siendo reutilizable. `design.show_clinicaclick_branding=false` puede ocultar la atribución visual, pero `marketing.campaign` conserva siempre la baja individual. Las bienvenidas de listas usan `applies_to=new_members_only`: crean una campaña con `source_list_id` y, si hay `import_batch_id`, clonan únicamente los elementos de ese lote para evitar envíos retrospectivos. Si incluyen email, su configuración guarda asunto y preheader propios junto a la plantilla y al remitente verificado.
- Desde el 24/09/2026, `mass_sends` admite `channels=['email']` y campañas combinadas. Los recursos `EmailSendingDomains`, `EmailSenderIdentities` y `MarketingEmailTemplates` pertenecen a un único scope de clínica o grupo. Preparar vuelve a exigir dominio, DKIM y MAIL FROM verificados, remitente activo, plantilla lista, variables completas, consentimiento y ausencia de supresión. Cada destinatario se materializa como `EmailMessage` con baja individual; pausar o cancelar se comprueba otra vez antes del broker.
- El transporte comercial real usa el broker tipado y `clinicaclick-marketing`; no existe fallback a una clave SES local. Los tres flags `EMAIL_ENABLED`, `EMAIL_MARKETING_ENABLED` y `EMAIL_BROKER_ENABLED` deben estar activos tanto en `.env` como en el entorno efectivo del worker. PM2 puede conservar valores anteriores: antes de un canary se comprueba `pm2 env`, no solo el archivo.
- En el namespace aislado `dev`, y solo si `EMAIL_BROKER_ENABLED` no está activo, registrar un dominio crea una identidad mock pendiente con tokens DNS deterministas. Nunca queda lista para enviar ni sustituye al proveedor. Si el broker está activo pero indisponible, o en staging/producción, el alta falla cerrada.
- Canary staging del 24/09/2026: un primer `EmailMessage` se cerró con `email_marketing_disabled` antes de SES por una variable PM2 antigua, sin `MessageId` ni eventos. Tras persistir el entorno correcto, `EmailMessage #215` / `JobRequest #120249` terminó `delivered` al primer intento, con eventos `send` y `delivery` y una sola fila para su `provider_message_id`.
...
- `GET /api/marketing/review-requests/automation-status?clinicId=<id>` es la lectura ligera para módulos que solo necesitan saber si **Conseguir reseñas** está activa. Reutiliza `authMiddleware`, `resolveReviewRequestSummaryScope` y por tanto los permisos del summary; exige una única clínica y devuelve `clinic_id`, `automation_enabled`, `automation_configured`, `configuration_errors` y el scope resuelto. Internamente solo llama a `getReviewAutomationTemplate(scope, { includeInactive: true })`: no construye candidatos, tratamientos, métricas, readiness de WhatsApp ni Perfil Google. Scope vacío, grupo, `all` o multiclínica devuelve `400 REVIEW_AUTOMATION_SINGLE_CLINIC_REQUIRED`.
...
- `PATCH /api/marketing/review-requests/automation` guarda, activa o desactiva una plantilla `AutomationFlowTemplatesV2` por clínica con `trigger_type=appointment_completed`. `save_configuration=true` permite guardar el momento clínico y la espera sin activarla. Para activarla exige además Perfil Google con `url_dejar_resena`, WhatsApp conectado, remitente (`review_sender_name`) y plantilla WABA aprobada concreta; el recordatorio de sin respuesta es opcional y no bloquea el primer envío. Si falta algo devuelve `409 review_automation_requirements_missing` con `warnings` (`template_not_approved`, `sender_name_missing`, etc.). Si recibe `scope=group:<id>`, ejecuta la misma operación clínica para cada sede del grupo y devuelve `group_result` con sedes actualizadas/activas/fallidas; no crea `review_request_after_completed__group_*`. La plantilla V2 encadena la espera inicial configurada tras `appointment_completed` -> `action/request_review` -> `delay/wait_response` de 24h. Si hay respuesta entra en `condition/field_check`, que comprueba `last_response_context.response_rating >= 5`; si es verdadero continúa por `action/review_followup` con `followup_kind=google_review`, y si es falso continúa por `action/review_followup` con `followup_kind=private_feedback`. Si no responde en 24h, intenta `action/request_review_reminder` con la plantilla `clinicaclick_recordatorio_resena_sin_respuesta` si existe activa; si no existe, salta a `action/review_no_response` y cierra el flujo sin bloquear el primer mensaje. La acción `request_review` conserva en su configuración `review_gift_enabled`, `review_gift_description`, `review_display_clinic_name`, `review_sender_name` y `review_team_photo_url`; el runtime los traslada a la lista generada automáticamente.
- Contrato fail-closed de reseñas automáticas 2026-08-06: una plantilla es ejecutable solo si es una copia publicada, no sistémica y del `clinic_id` exacto, con `trigger_config.managed_feature=review_request`, `configured=true`, `configuration_version>=1`, fuente válida, espera inicial coherente entre `trigger_config` y `delay/fixed`, `action/request_review`, remitente y `whatsapp_template_id` concreto. `appointmentAutomationV2Runtime` aplica el mismo validador al seleccionar y al encolar; nunca cae a la base global/sistema ni a una copia de otra sede. La lista automática guarda `review_automation_template_id` y tanto el primer envío como el recordatorio vuelven a validar esa identidad. `20260806110000-harden-explicit-review-automation-config` marca las configuraciones completas, desactiva todas las copias y retira `auto_solicitar_resena` como default de trigger: una migración o propagación nunca opta a una clínica dentro del envío automático.
...
- En envíos de prueba de reseñas, `sendTest` persiste antes de enviar los campos editados en el paso `Mensaje` (`review_display_clinic_name`, `review_sender_name`, `review_team_photo_url`, color y `review_team_members_text`) y los copia también a la metadata del mensaje disparador. Esto evita carreras: si el paciente de prueba responde `5` inmediatamente, el follow-up lee el copy actualizado aunque la UI siga en el stepper. Las pruebas se deduplican por `trigger_message_id`, no por contacto/lista global, para poder repetir un test sobre el mismo paciente de ejemplo sin que una respuesta anterior bloquee la nueva. Desde 2026-08-06, `review_sender_name` es obligatorio al crear o preparar la solicitud, enviar la prueba y arrancar la cola. Se normaliza mediante `review-sender-policy` y un valor vacío devuelve `409 review_sender_name_missing`; no se completa silenciosamente con `Recepción`.
- `sendTest` no debe generar un disparador de reseña sin configuración humana. Puede recuperar remitente, foto o miembros desde la lista actual y, si hace falta, desde `last_request_template`, pero el remitente recuperado debe ser explícito y no un fallback genérico. Esos valores se copian a `metadata.review_*` del mensaje disparador para que el follow-up de `5/5` pueda renderizar el texto correcto.
...
- En listados de automatizaciones V2 con scope de clínica/grupo, la base global de reseñas no debe mostrarse como automatización operativa. Las filas publicadas deprecadas/inactivas tampoco se muestran en `Todos`. La UI materializa una única tarjeta gestionada por clínica para reseñas y otra para respuesta automática a leads; ambas permanecen visibles activas, pausadas o pendientes de configuración y derivan a su superficie de configuración. Esto evita que una base de catálogo parezca activa y que la misma función salte entre secciones al cambiar de estado.
