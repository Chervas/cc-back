...
- `businessProfileReviewsSync` es el refresco ligero para conversión de reseñas: por defecto corre cada 15 minutos (`JOBS_BUSINESS_PROFILE_REVIEWS_SCHEDULE`, configurable), solo llama a la API v4 de reseñas, limita páginas con `LOCAL_REVIEWS_SYNC_MAX_PAGES` (2 por defecto) y solo encola conciliación para reseñas nuevas en esa pasada. El sync completo diario sigue siendo el que consolida métricas, publicaciones y reintentos amplios;
