...
- `COMPETITION_GOOGLE_ADS_TRANSPARENCY_ENABLED=false` desactiva Google Ads Transparency sin tocar el resto del informe. El fast-path no usa credencial y está acotado por `COMPETITION_GOOGLE_ADS_TRANSPARENCY_FAST_PATH_ENABLED`, `..._FAST_AD_LIMIT` y `..._TIMEOUT_MS`. El lote oficial requiere `GOOGLE_APPLICATION_CREDENTIALS` —o JSON/base64 equivalente fuera de Git— y `COMPETITION_GOOGLE_ADS_TRANSPARENCY_WEEKLY_ENABLED=true` solo en el worker líder. `COMPETITION_GOOGLE_ADS_TRANSPARENCY_MAX_BYTES_BILLED` fija el techo duro; su valor seguro por defecto es `30000000000`.
