...
- `GET /api/marketing/reports/competition/ai-visibility` exige una clínica concreta, revalida `marketingScopeAccess` y, al entrar el usuario en Informes, garantiza cuatro consultas canónicas del sistema: mejor clínica local, mejor dentista local, clínica recomendada y clínica con buenas reseñas. No depende de texto libre ni de que el usuario pulse un botón. Este GET es el único disparador automático: `getOverview()` usa `autoStart=false` por defecto, `GET /:runId` solo hace polling y no existe cron diario/semanal de proveedores.
