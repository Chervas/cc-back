...
function credentialsFixture() {
  const state = { markers: [], scMarkers: [], gaMarkers: [], propertyMarkers: [], rows: new Map(), loads: 0, updates: 0, tokenReads: 0, provider: [], logs: [], failMetadata: false };
  const blocked = row => [...state.markers, ...state.scMarkers, ...state.gaMarkers, ...state.propertyMarkers].some(b => Number(b.google_connection_id) === row?.id || b.google_user_id === row?.googleUserId);
