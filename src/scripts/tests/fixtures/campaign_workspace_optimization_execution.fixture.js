...
function matches(row, where) {
  return Reflect.ownKeys(where).every(key => {
    ...
    return Reflect.ownKeys(value).every(op => op === Op.ne ? row[key] !== value[op]
      : op === Op.in ? value[op].includes(row[key]) : op === Op.gt ? row[key] != null && +new Date(row[key]) > +new Date(value[op]) : false);
  });
}
...
function fixture(provider = 'google_ads', action = 'adjust_bids') {
  ...
  const deps = { models, namespace: 'isolated-test', now: () => state.now,
    ...
    lockAccounts: async () => {},
    authorize: async ({ setting: current, campaign }) => {
      state.calls.authorize++;
      if (!state.permitted) throw error('workspace_optimization_permissions_required');
      if (current.activation.optimization.status !== 'active') throw error('workspace_optimization_authorization_required');
      return { entry: { targets: [target] }, limits: { cooldown_hours: 24, monthly_limit_cents: 100000 },
    ...
    enqueue: async request => {
      ...
      const job = { id: state.jobs.size + 1, ...structuredClone(request) }; state.jobs.set(job.id, job); return { job, created: true };
    },
  ...
  return { state, setting, change, evidence, deps,
    ...
    run: (id = 1, payloadPatch = {}) => { const job = state.jobs.get(id); return runOptimizationAdjustmentJob({ ...job.payload, ...payloadPatch }, job, deps); },
