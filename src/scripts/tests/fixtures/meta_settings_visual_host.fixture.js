...
module.exports=async({app,server,token,groupMode=false})=>{
  const selectedFront=process.env.META_QA_FRONT_ROOT;
  if(selectedFront&&!path.isAbsolute(selectedFront))throw Error('META_QA_FRONT_ROOT must be absolute');
  const front=fs.realpathSync(selectedFront||path.resolve(__dirname,'../../../../../front-dev')),req=createRequire(path.join(front,'package.json'));
  // A selective CRM candidate need not contain the pending Google/workspace UI.
  // Do not import those components from DEV while claiming to test the candidate.
  const googleRevocation=fs.existsSync(path.join(front,'src/app/modules/admin/pages/settings/connected-accounts/google-revocation-status.component.ts'));
  const workspace=fs.existsSync(path.join(front,'src/app/modules/admin/apps/marketing/campaign-workspace/campaign-account-selection.model.ts'));
...
${googleRevocation?"import {GoogleRevocationStatusComponent} from './src/app/modules/admin/pages/settings/connected-accounts/google-revocation-status.component';":''}
...
${workspace?"import {accountDiscoveryStep,workspaceErrorMessage} from './src/app/modules/admin/apps/marketing/campaign-workspace/campaign-account-selection.model';":"const accountDiscoveryStep=undefined,workspaceErrorMessage=undefined;"}
...
${googleRevocation?'GoogleRevocationStatusComponent.ctorParameters=()=>[HttpClient,ChangeDetectorRef].map(type=>({type}));':''}
...
const groupName=new BehaviorSubject(${groupMode?"'Grupo ficticio'":'null'});
const filter={selectedClinicId$:selected,selectedGroupName$:groupName,filteredClinics$:filtered,getCurrentClinicFilter:()=>selected.value,getCurrentSelectedGroupName:()=>groupName.value,getCurrentFilteredClinics:()=>filtered.value,isCurrentUserAdmin:()=>false};
...
select(id){groupName.next(null);selected.next(String(id));filtered.next([clinics.find(c=>c.id_clinica===id)]);this.cdr.markForCheck();}
selectGroup(){groupName.next('Grupo ficticio');selected.next('59,71');filtered.next(clinics);this.cdr.markForCheck();} }
...
{provide:ClinicFilterService,useValue:filter},{provide:UserService,useValue:{user$:of(null)}},{provide:AuthService,useValue:{getCurrentUser:()=>of({id_usuario:91002,nombre:'Usuario ficticio'}),get accessToken(){return window.QA_TOKEN;}}},
...
provideTransloco({config:{availableLangs:['es'],defaultLang:'es',reRenderOnLangChange:true,prodMode:true},loader:Loader})]});`;
  ...
  const styles=selectedFront?path.join(front,'dist/fuse'):'/home/ubuntu/www/front-dev-preview',cssName=fs.readdirSync(styles).find(v=>/^styles(?:[.-].+)?\.css$/.test(v));assert(cssName);
