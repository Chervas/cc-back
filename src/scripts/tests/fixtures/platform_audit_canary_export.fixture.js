...
module.exports=async({models,report})=>{
  ...
  const records=rows.filter(row=>[20,21,22,23,24,25].includes(JSON.parse(row.body).version));assert(records.length>0&&records.length<=1000);
  for(const record of records){const event=unpack(record).event;
    if(event.version===25){
      assert.equal(event.scope.type,'clinic');assert(['71','72'].includes(event.scope.id));
      assert.equal(event.initiatorUserId,'91002');assert.equal(event.connectionRef,'connection:test');
      assert.equal(event.assetRef,'gbp:123:456');assert.equal(event.mappingId,'51');
    }else{
