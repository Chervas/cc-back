...
test('fresh view notifications survive bus failure, isolate phones and never replay business actions',async()=>{
 await withIsolatedCampaignMysql(async({sql,report})=>{
  ...
  report.checks.push('Redis failure preserves notification','exact clinic and phone','history and cutoff excluded','import holds do not hide reception','created/read notifications','no message text or automation trigger','configuration race','updatedAt preserved');
 });
});
