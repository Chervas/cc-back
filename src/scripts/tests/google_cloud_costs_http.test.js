...
const { createService } = require('../../services/googleCloudCosts.service');
...
async function fixture(t) {
  ...
  const service = createService({ now: () => new Date('2026-09-20T12:00:00Z'), collect: forbidden,
  ...
  const jobs = load('controllers/metasync.jobs.controller.js', {
    ...
    '../../models': {}, '../services/googleCloudCosts.service': service,
  });
  ...
  return { reads: () => reads, request: (actor, query = '') => new Promise((resolve, reject) => {
    ...
    http.get({ host: '127.0.0.1', port: server.address().port, agent,
      path: '/api/metasync/jobs/usage/google-cloud/costs' + query,
...
test('actual controller returns private closed cache to both technical admins and validates month', async t => {
  ...
  for (const actor of [1, 44]) {
    ...
    assert.equal(r.headers['cache-control'], 'private, no-store'); assert.equal(r.body.snapshot.net, '0'); assert.equal(r.body.month, '2026-08');
  }
