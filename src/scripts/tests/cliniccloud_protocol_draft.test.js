...
for (const kind of ['manual-facial', 'manual-capilar', 'manual-obesidad']) test(`${kind} can create only a traceable actorless unassociated draft inside an operator transaction`, async () => {
  const h = harness();
  const payload = { ...h.plan.payload, source: h.plan.payload.source.replace('manual-corporal', kind) };
  const importedSource = { actor_kind: 'system_import', source_system: 'cliniccloud', source_sha256: h.plan.source_sha256 };
  ...
  for (const update of [{ status: 'approved' }, { kind: 'aftercare' }, { treatment_ids: [1] },
    { source: payload.source.replace(kind, 'arbitrary-import') },
