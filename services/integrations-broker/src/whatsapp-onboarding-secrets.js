...
const { BrokerError, fail } = require('./errors'); const C = require('./whatsapp-onboarding-contract');
const { tokenText } = require('./whatsapp-secrets');
function createWhatsappOnboardingSecrets({ client, accountId, prefix, kmsKeyArn }) {
  ...
  async function slot(binding, signal) {
    ...
    if (!C.exact(v, ['version', 'provider', 'connectionRef', 'scopeKey', 'appId']) || v.version !== 1 || v.provider !== 'meta-whatsapp-onboarding-slot'
      || v.connectionRef !== binding.connectionRef || v.scopeKey !== b.scopeKey || v.appId !== b.appId) fail('secret_unavailable');
    return b;
  }
  ...
  return {
    ...
    async withApplication(binding, work, signal) {
      ...
      try {
        const b = await slot(binding, signal); const r = await read(binding.clientSecretArn, b.appVersionId, 'AWSCURRENT', signal);
    ...
    async stage(binding, flow, encoded, signal) {
      let body;
      try {
        if (!Buffer.isBuffer(encoded?.body) || encoded.body.length > 32768 || C.hash(encoded.body) !== encoded.digest) fail('invalid_request');
...
module.exports = { createWhatsappOnboardingSecrets };
