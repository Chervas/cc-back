  ...
  return async (req, res) => {
    ...
    try {
      ...
      if (transportProfile === 'ai') {
        ...
        if (activeRequests >= aiLimits.MAX_CONCURRENT_REQUESTS
          || pendingBytes + reservedBytes > aiLimits.MAX_PENDING_REQUEST_BYTES) throw new BrokerError('rate_limited');
      ...
      if (transportProfile === 'ai' && !res.headersSent) res.setHeader('Connection', 'close');
