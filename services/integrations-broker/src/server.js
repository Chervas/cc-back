...
function handler(broker, { transportProfile = 'default' } = {}) {
  if (!['default', 'ai', 'email'].includes(transportProfile)) throw Error('Invalid transport profile');
  const limits = transportProfile === 'ai' ? aiLimits : transportProfile === 'email' ? emailLimits : null;
...
function createServer(broker, tls, options = {}) {
  ...
  server.headersTimeout = 10000; server.timeout = options.transportProfile === 'ai' ? aiLimits.MAX_TIMEOUT_MS + 5000
    : options.transportProfile === 'email' ? emailLimits.MAX_TIMEOUT_MS + 5000 : 20000;
