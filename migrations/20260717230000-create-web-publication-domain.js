...
const SCOPE_TABLES = new Set(['WebDomains', 'WebWordpressInstallations', 'WebPublications']);
...
function inventoryTableName(inventory, expectedName) {
  ...
  if (matches.length > 1) {
    throw migrationError(
      'web_publication_migration_ambiguous_table',
...
function assertCapabilities(queryInterface) {
  ...
  if (missing.length) {
    throw migrationError(
      'web_publication_migration_introspection_unavailable',
      `No se puede validar de forma segura el dominio de publicación web: faltan ${missing.join(', ')}.`,
...
function assertDependencies(inventory) {
  ...
  if (missing.length) {
    throw migrationError(
      'web_publication_migration_missing_dependency',
      `No se puede crear el dominio de publicación web; faltan tablas previas: ${missing.join(', ')}.`,
...
function assertColumns(tableName, actual, expected) {
  ...
  if (missing.length) {
    throw migrationError(
      'web_publication_migration_incompatible_table',
  ...
  if (mismatches.length) {
    throw migrationError(
      'web_publication_migration_incompatible_column',
...
async function addIndex(queryInterface, tableName, fields, options) {
  ...
  if (named.length > 1) {
    throw migrationError(
      'web_publication_migration_incompatible_index',
...
async function ensureCheckConstraint(queryInterface, Sequelize, tableName, definition) {
  ...
  if (named.length > 1) {
    throw migrationError(
      'web_publication_migration_incompatible_constraint',
  ...
  if (violationCount > 0) {
    throw migrationError(
      'web_publication_migration_constraint_data_violation',
...
async function repairLegacyScopeForeignKey(queryInterface, tableName, contract, current) {
  ...
  if (dialectOf(queryInterface) !== 'mysql' || typeof queryInterface?.sequelize?.query !== 'function') {
    throw migrationError(
      'web_publication_migration_foreign_key_repair_unavailable',
  ...
  try {
    ...
    const error = migrationError(
      'web_publication_migration_foreign_key_repair_failed',
...
async function ensureForeignKeys(queryInterface, tableName, definition) {
  ...
  for (const contract of contracts) {
    ...
    if (matches.length > 1) {
      throw migrationError(
        'web_publication_migration_incompatible_foreign_key',
...
async function createOrAssert(queryInterface, tableName, definition) {
...
async function assertCheckCompatibleForeignKeys(queryInterface, tableName, columns) {
  ...
  for (const column of columns) {
    ...
    if (!foreignKey || !compatibleAction(foreignKey.onUpdate, 'RESTRICT')) {
      throw migrationError(
        'web_publication_migration_incompatible_foreign_key_action',
...
const scopeColumns = (Sequelize) => ({
  scope_type: { type: Sequelize.ENUM('clinic', 'group'), allowNull: false },
  clinica_id: {
    type: Sequelize.INTEGER,
    allowNull: true,
    references: { model: 'Clinicas', key: 'id_clinica' },
    // Los IDs de scope son estables. MySQL no permite CHECK + ON UPDATE CASCADE sobre estas columnas (3823).
    onUpdate: 'RESTRICT',
    onDelete: 'RESTRICT',
  },
  grupo_clinica_id: {
...
const timestamps = (Sequelize, { updated = true } = {}) => ({
...
module.exports = {
  ...
  async up(queryInterface, Sequelize) {
    if (typeof queryInterface?.showAllTables !== 'function') {
      throw migrationError(
        'web_publication_migration_introspection_unavailable',
        'No se puede comprobar el inventario antes de migrar el dominio de publicación web.'
      );
    }
    const inventory = await tableInventory(queryInterface);
    assertDependencies(inventory);
    assertCapabilities(queryInterface);
    await createOrAssert(queryInterface, 'WebDomains', {
      id: { type: Sequelize.STRING(36), allowNull: false, primaryKey: true },
      ...scopeColumns(Sequelize),
      host: { type: Sequelize.STRING(253), allowNull: false },
      kind: {
        type: Sequelize.ENUM('clinicaclick_hosted', 'custom_domain'),
        allowNull: false,
      },
      status: {
        type: Sequelize.ENUM('pending_dns', 'pending_tls', 'ready', 'failed', 'retired'),
        allowNull: false,
        defaultValue: 'pending_dns',
      },
      ownership_token_hash: { type: Sequelize.STRING(64), allowNull: false },
      provider_reference_hash: { type: Sequelize.STRING(64), allowNull: true },
      expected_dns: { type: Sequelize.JSON, allowNull: false },
      verification: { type: Sequelize.JSON, allowNull: false },
      tls: { type: Sequelize.JSON, allowNull: false },
      commercial_mode: {
        type: Sequelize.ENUM('included', 'paid_domain'),
        allowNull: false,
    ...
    await assertCheckCompatibleForeignKeys(queryInterface, 'WebDomains', ['clinica_id', 'grupo_clinica_id']);
    await ensureCheckConstraint(queryInterface, Sequelize, 'WebDomains', {
      ...CLINIC_GROUP_SCOPE_CHECK,
      name: 'chk_web_domains_scope',
    });
    await addIndex(queryInterface, 'WebDomains', ['host'], {
      name: 'uniq_web_domains_host', unique: true,
    });
    await addIndex(queryInterface, 'WebDomains', ['clinica_id', 'status'], {
      name: 'idx_web_domains_clinic_status',
    });
    await addIndex(queryInterface, 'WebDomains', ['grupo_clinica_id', 'status'], {
      name: 'idx_web_domains_group_status',
    });

    await createOrAssert(queryInterface, 'WebWordpressInstallations', {
      id: { type: Sequelize.STRING(36), allowNull: false, primaryKey: true },
      ...scopeColumns(Sequelize),
      site_url: { type: Sequelize.STRING(2048), allowNull: false },
      site_url_hash: { type: Sequelize.STRING(64), allowNull: false },
      token_hash: { type: Sequelize.STRING(64), allowNull: false },
      token_prefix: { type: Sequelize.STRING(16), allowNull: false },
      status: {
        type: Sequelize.ENUM('pending', 'connected', 'outdated', 'revoked'),
        allowNull: false,
        defaultValue: 'pending',
      },
      plugin_version: { type: Sequelize.STRING(32), allowNull: true },
      capabilities: { type: Sequelize.JSON, allowNull: false },
      reported_state: { type: Sequelize.JSON, allowNull: false },
      public_key_id: { type: Sequelize.STRING(64), allowNull: false },
      last_seen_at: { type: Sequelize.DATE, allowNull: true },
      last_artifact_hash: { type: Sequelize.STRING(64), allowNull: true },
      desired_sequence: { type: Sequelize.INTEGER.UNSIGNED, allowNull: false, defaultValue: 0 },
      desired_state_hash: { type: Sequelize.STRING(64), allowNull: true },
      version: { type: Sequelize.INTEGER.UNSIGNED, allowNull: false, defaultValue: 1 },
      created_by_user_id: userForeignKey(Sequelize),
      updated_by_user_id: userForeignKey(Sequelize),
      ...timestamps(Sequelize),
    ...
    await assertCheckCompatibleForeignKeys(
      queryInterface,
      'WebWordpressInstallations',
      ['clinica_id', 'grupo_clinica_id']
    );
    await ensureCheckConstraint(queryInterface, Sequelize, 'WebWordpressInstallations', {
      ...CLINIC_GROUP_SCOPE_CHECK,
      name: 'chk_web_wordpress_installations_scope',
    });
    await addIndex(queryInterface, 'WebWordpressInstallations', ['site_url_hash'], {
      name: 'uniq_web_wordpress_site_url_hash', unique: true,
    });
    await addIndex(queryInterface, 'WebWordpressInstallations', ['token_hash'], {
      name: 'uniq_web_wordpress_token_hash', unique: true,
    });
    await addIndex(queryInterface, 'WebWordpressInstallations', ['clinica_id', 'status'], {
      name: 'idx_web_wordpress_clinic_status',
    });
    await addIndex(queryInterface, 'WebWordpressInstallations', ['grupo_clinica_id', 'status'], {
      name: 'idx_web_wordpress_group_status',
    });

    await createOrAssert(queryInterface, 'WebPublications', {
      id: { type: Sequelize.STRING(36), allowNull: false, primaryKey: true },
      project_id: {
      ...
      ...scopeColumns(Sequelize),
      channel: {
        type: Sequelize.ENUM('clinicaclick_hosted', 'wordpress', 'custom_domain'),
        allowNull: false,
      },
      domain_id: {
        type: Sequelize.STRING(36), allowNull: true,
        references: { model: 'WebDomains', key: 'id' }, onUpdate: 'CASCADE', onDelete: 'RESTRICT',
      },
      wordpress_installation_id: {
        type: Sequelize.STRING(36), allowNull: true,
        references: { model: 'WebWordpressInstallations', key: 'id' }, onUpdate: 'CASCADE', onDelete: 'RESTRICT',
      },
      host: { type: Sequelize.STRING(253), allowNull: false },
      path: { type: Sequelize.STRING(512), allowNull: false, defaultValue: '/' },
      status: {
        type: Sequelize.ENUM('draft', 'pending', 'publishing', 'published', 'failed', 'rolling_back', 'retired'),
        allowNull: false,
        defaultValue: 'draft',
      },
      desired_revision_id: {
      ...
      configuration: { type: Sequelize.JSON, allowNull: false },
      health: { type: Sequelize.JSON, allowNull: false },
      ...
      last_healthy_at: { type: Sequelize.DATE, allowNull: true },
      created_by_user_id: userForeignKey(Sequelize),
      updated_by_user_id: userForeignKey(Sequelize),
      ...timestamps(Sequelize),
      retired_at: { type: Sequelize.DATE, allowNull: true },
    });
    await assertCheckCompatibleForeignKeys(queryInterface, 'WebPublications', ['clinica_id', 'grupo_clinica_id']);
    await ensureCheckConstraint(queryInterface, Sequelize, 'WebPublications', {
      ...CLINIC_GROUP_SCOPE_CHECK,
      name: 'chk_web_publications_scope',
    });
    await addIndex(queryInterface, 'WebPublications', ['host', 'path'], {
      name: 'uniq_web_publications_host_path', unique: true,
    });
    await addIndex(queryInterface, 'WebPublications', ['project_id', 'status'], {
      name: 'idx_web_publications_project_status',
    });
    await addIndex(queryInterface, 'WebPublications', ['clinica_id', 'status'], {
      name: 'idx_web_publications_clinic_status',
    });
    await addIndex(queryInterface, 'WebPublications', ['grupo_clinica_id', 'status'], {
      name: 'idx_web_publications_group_status',
    });
    await addIndex(queryInterface, 'WebPublications', ['job_request_id'], {
      name: 'idx_web_publications_job_request',
    });
    await addIndex(queryInterface, 'WebPublications', ['wordpress_installation_id'], {
      name: 'uniq_web_publications_wordpress_installation', unique: true,
    });

    await createOrAssert(queryInterface, 'WebPublicationDeployments', {
      id: { type: Sequelize.STRING(36), allowNull: false, primaryKey: true },
      publication_id: {
        type: Sequelize.STRING(36), allowNull: false,
      ...
      action: {
        type: Sequelize.ENUM('publish', 'rollback', 'retire', 'healthcheck'),
        allowNull: false,
      },
      status: {
        type: Sequelize.ENUM('queued', 'running', 'verified', 'failed', 'superseded'),
        allowNull: false,
        defaultValue: 'queued',
      },
      expected_publication_version: { type: Sequelize.INTEGER.UNSIGNED, allowNull: false },
      storage: { type: Sequelize.JSON, allowNull: false },
      result: { type: Sequelize.JSON, allowNull: false },
      error_code: { type: Sequelize.STRING(128), allowNull: true },
      error_details: { type: Sequelize.JSON, allowNull: true },
      job_request_id: {
        type: Sequelize.INTEGER.UNSIGNED, allowNull: true,
        references: { model: 'JobRequests', key: 'id' }, onUpdate: 'CASCADE', onDelete: 'SET NULL',
      },
    ...
    await addIndex(queryInterface, 'WebPublicationDeployments', ['publication_id', 'sequence'], {
      name: 'uniq_web_publication_deployments_sequence', unique: true,
    });
    await addIndex(queryInterface, 'WebPublicationDeployments', ['publication_id', 'status', 'created_at'], {
      name: 'idx_web_publication_deployments_status',
    });
    await addIndex(queryInterface, 'WebPublicationDeployments', ['job_request_id'], {
      name: 'idx_web_publication_deployments_job_request',
    });
  },
  ...
  async down(queryInterface) {
    if (typeof queryInterface?.showAllTables !== 'function' || typeof queryInterface?.dropTable !== 'function') {
      throw migrationError(
        'web_publication_migration_introspection_unavailable',
        'No se puede revertir de forma segura la migración de publicación web.'
      );
    }
