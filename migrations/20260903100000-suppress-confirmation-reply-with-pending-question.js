...
const SNAPSHOT_KEY = 'suppress_confirmation_reply_with_pending_question_v1';
  ...
  async up(queryInterface) {
    await queryInterface.sequelize.transaction(async (transaction) => {
      ...
      for (const familyRows of groupByPublicId(rows).values()) {
        ...
        const patched = markCanonicalConfirmationReplySuppression(
        ...
        if (!validation.ok) {
          ...
          throw new Error(
            `pending_question_reply_suppression_validation_failed:${latestPublished.public_id}:${codes.join(',')}`,
          );
        }
        candidates.push({
          source: latestPublished,
          nodes: patched.nodes,
          marked: patched.marked,
